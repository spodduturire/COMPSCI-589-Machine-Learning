...
class NodeNumeric:
    def __init__(self, attribute_name=None, left=None, right=None, threshold=None, information_gain=None, value=None):
        self.attribute_name = attribute_name
        self.left = left
        self.right = right
...
class DecisionTreeClassifierNumeric:
    def __init__(self, min_samples_split=10):
        self.root=None
        self.min_samples_split = min_samples_split

    def build_tree(self, dataset):
        X, Y = dataset.loc[:, dataset.columns != '# class'], dataset.iloc[:,-1:]
        if len(X)>=self.min_samples_split:
            best_split= self.get_best_split(dataset)
            if best_split["information_gain"]>0:
                left_subtree = self.build_tree(best_split["dataset_left"])
                right_subtree = self.build_tree(best_split["dataset_right"])
                return NodeNumeric(best_split["attribute_name"], left_subtree,
                             right_subtree, best_split["threshold"], best_split["information_gain"])

        leaf_value = self.calculate_leaf_target(Y)
        return NodeNumeric(value=leaf_value)
    ...
    def get_best_split(self, dataset):
        ...
        for attribute in random_attributes_list:
            ...
            if len(dataset[attribute].unique()) > 1:
                # three-way splitting here
                for threshold in np.unique(dataset[attribute]):
                    left, right = dataset.loc[dataset[attribute]<=threshold],dataset.loc[dataset[attribute]>threshold]
                    if len(left)>0 and len(right)>0:
                        # calculate information gain
                        gain = self.get_information_gain(target,left.iloc[:,0],right.iloc[:,0])
                        # checks for attribute with highest information gain
                        if gain > max_information_gain:
                            best_split["attribute_name"] = attribute
                            best_split["dataset_left"] = left
    ...
    def get_information_gain(self, parent, left, right):
        gain = 0.0
        gain = self.get_entropy(parent) - ((len(left)/len(parent))*self.get_entropy(left) + (len(right)/len(parent))*self.get_entropy(right))
        return gain
...
data["# class"].unique
data_target_0 = data.loc[data.iloc[:,0] == 1]
data_target_1 = data.loc[data.iloc[:,0] == 2]
data_target_2 = data.loc[data.iloc[:,0] == 3]
splits_0 = []
j = round(len(data_target_0)/10)
for i in range(0,len(data_target_0),j):
    if (len(data_target_0) - i) < (2 * (j-1)):
        j = len(data_target_0) - j
    splits_0.append(data_target_0.iloc[i : i+j])
splits_1 = []
j = round(len(data_target_1)/10)
for i in range(0,len(data_target_1),j):
    if (len(data_target_1) - i) < (2 * (j-1)):
        j = len(data_target_1) - j
    splits_1.append(data_target_1.iloc[i : i+j])
splits_2 = []
j = round(len(data_target_2)/10)
for i in range(0,len(data_target_2),j):
    if (len(data_target_2) - i) < (2 * (j-1)):
        j = len(data_target_2) - j
    splits_2.append(data_target_2.iloc[i : i+j])
splits = []
for i in range(10):
    res = pd.concat([splits_0[i],splits_1[i],splits_2[i]])
    splits.append(res)
...
def bootstrap_numeric(train_set,test_set,value):
    bootstrapSetAccuracies = []
    X_test, Y_test = test_set.loc[:, test_set.columns != '# class'], test_set.iloc[:,0]
    ...
    for i in range(value):
        ...
        random_elements = list(np.random.randint(low = 0, high = len(train_set), size = len(train_set)))
        boot_df = boot_df.append(train_set.iloc[random_elements])
        X_train, Y_train = boot_df.loc[:, boot_df.columns != '# class'], boot_df.iloc[:,0]
        dtc = DecisionTreeClassifierNumeric()
        dtc.fit(X_train,Y_train)
        prediction = dtc.predict(X_test)
        Predictions.append(prediction)
...
for value in values:
    set_predictions = []
    print(f"For bootstrap value {value}: ")
    for i in range(len(train_sets)):
        print(f"For test set {i}")
        Predictions = bootstrap_numeric(train_sets[i],test_sets[i], value)
        final_predictions = []
        for j in range(len(test_sets[i])):
            l = [predict[j][0] for predict in Predictions] #REMOVED predict[j][0]
            final_predictions.append(mode(l))
        set_predictions.append(final_predictions)
    value_predictions[value] = set_predictions

def get_metrics_numeric(y_test,y_pred):
    tp_1 = 0
    fp_1 = 0
    fn_1 = 0
    ...
    for i in range(len(y_test)):
        if y_test[i] == y_pred[i] and y_pred[i] == 1:
            ...
            fn_1 += 1
    for i in range(len(y_test)):
        if y_test[i] == y_pred[i] and y_pred[i] == 2:
            tp_2 += 1
        elif y_pred[i] == 2:
            fp_2 += 1
        elif (y_test[i] != y_pred[i]) and (y_test[i] == 2) :
            fn_2 += 1
    for i in range(len(y_test)):
        if y_test[i] == y_pred[i] and y_pred[i] == 3:
            tp_3 += 1
        elif y_pred[i] == 3:
            fp_3 += 1
        elif (y_test[i] != y_pred[i]) and (y_test[i] == 3) :
            fn_3 += 1
    recall_count1 = 0
    recall_count2 = 0
    recall_count3 = 0
...
def get_f1_score_numeric(precision,recall):
    return 2*(precision*recall)/(precision+recall)
...
for i in range(len(train_sets)):
    ...
    print(f"For Cross-Validation Split {i}")
    for value in values:
        accuracy = accuracy_score(test_sets[i].iloc[:,0].values.tolist(),value_predictions[value][i])
        our_precision, our_recall = get_metrics_numeric(test_sets[i].iloc[:,0].values.tolist(),value_predictions[value][i])
        f1_score = get_f1_score_numeric(our_precision,our_recall)
        print(f"Accuracy for value {value} is : {accuracy}")
        print(f"Precision for value {value} is : {our_precision}")
        print(f"Recall for value {value} is : {our_recall}")
        print(f"F1 score fore value {value} is : {f1_score}")
...
def calculate_deviation_numeric(dataset_accuracies, values):
    metrics_per_value = {}
    for j, value in enumerate(values):
        all_accuracies_per_value = []
        for i in range(10):
...
values = [1,5,10,20,30,40,50]
A = calculate_deviation_numeric(dataset_accuracies,values)
B = calculate_deviation_numeric(dataset_precision,values)
C = calculate_deviation_numeric(dataset_recall,values)
D = calculate_deviation_numeric(dataset_f1_score,values)
